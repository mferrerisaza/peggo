...
RSpec.feature "Shares features", type: :feature do
  ...
  context "on owners show" do
    context "actions compleated correctly" do
      scenario "user creates a share on owners show", js: true do
        user = FactoryBot.create(:user, :with_owners)
        building = FactoryBot.create(:building, :with_propierties)
        building.owners << user.owners
        owner = user.owners.first
        ...
        sign_in_as user
        visit "/buildings/#{building.id}/owners/#{owner.id}"
        page.driver.browser.manage.window.resize_to(1024, 768)
        aggregate_failures do
          expect(page).to have_content(owner.name)
          expect(page).to have_content("Agregar propiedad")
          expect(page).to_not have_css("#new_share")
        end
        click_link "Agregar propiedad"
        expect(page).to have_css('#new_share')
        expect do
          select property.full_name, from: "share_property_id"
          fill_in "share_ownerability_percentage", with: "1"
          fill_in "share_payment_percentage", with: "1"
          within "#new_share" do
            find(".submit-share-form").click
          ...
          expect(page).to_not have_css("#new_share")
          expect(page).to have_content("Agregar propiedad")
        end.to change(owner.shares, :count).by 1
      end
      ...
      scenario "user updates a share on owners show", js: true do
        ...
        building.properties << share.property
        user.buildings << building
        ...
        expect do
          ...
          expect(page).to_not have_css("#edit_share_#{share.id}")
          share.reload
        end.to change(share, :property_id).from(original_property.id).to(property.id)
      end
      ...
      scenario "user deletes a share on owners show", js: true do
        ...
        expect do
          ...
          page.driver.browser.switch_to.alert.accept
          expect(page).to_not have_css("div[data-share-id='#{share.id}']")
          expect(page).to_not have_content(share.property.full_name)
        end.to change(owner.shares, :count).by(-1)
      end

      scenario "user creates and edit a share on owners show and then reopen and closes edit form", js: true do
        user = FactoryBot.create(:user, :with_owners)
        building = FactoryBot.create(:building, :with_propierties)
        building.owners << user.owners
        owner = user.owners.first
      ...
      scenario "user try to create a share on owners show and fails", js: true do
        ...
        expect do
          ...
          aggregate_failures do
            ...
            expect(page).to have_css(".is-invalid")
            expect(page).to have_css("#new_share")
            expect(page).to_not have_content("Agregar propiedad")
          end
        end.to_not change(owner.shares, :count)
